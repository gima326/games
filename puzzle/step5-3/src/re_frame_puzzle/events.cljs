   ...
   (vec
    (for [idx (range (* size size))
      ...
      {:id idx
       ...
       :text (if last?
               ;; ゲーム中は「■」が表示されている。中断時は「▶」が表示されている。
               (if flg TEXT-PLAY TEXT-PAUSE)
               ;; 中断時には数字を非表示にする。
               (if flg val TEXT-HIDE))}
      ))))
...
(re-frame/reg-event-db
 ...
 (fn [db [_ size]]
   (let [goal_state (initTILES size)
         [shuffled cnt] (my-shuffle2
                         ;;goal_state
                         (initTILES size false)
                         size
                         )]
...
;;   (js/alert (str "selected: " type))
   ...
   ;; ローカルストレージの :type を上書きしたい
   (db/type->local-store {:type type})
...
(re-frame/reg-event-db
 ...
 (fn [db _]
   (let [status (:status db)
         status-new (if (= status TEXT-PAUSE)
                      TEXT-PLAY
                      TEXT-PAUSE)

         tiles (:tiles db)
         tiles-cnt (count tiles)
         tiles-new (vec
                    (for [t tiles
                          :let [val (:value t)
                                text-new
                                (if (< val tiles-cnt)
                                  ;; 数字のマス
                                  (if (= status TEXT-PAUSE) val TEXT-HIDE)
                                  ;; 空欄のマス
                                  status-new)]]

                      (assoc t :text text-new)))]
...
(re-frame/reg-event-fx
 ...
 [
  ...
  (re-frame/inject-cofx ::db/local-store-type)
 ...
 (fn [{:keys [db local-store-log local-store-size local-store-type]} _]
   ...
   (let [ls-size (:size local-store-size)
         ...
         ls-type (:type local-store-type)

         ;; 初回起動時にはローカルストレージが未登録なので、ガードを掛けている。
         size (if (nil? ls-size) 3 ls-size)
         type (if (nil? ls-type) 0 ls-type)

         log (if (nil? ls-log)
               []
               ;; 最新のログを SVG で表示する上限の 13 件分取得している。
               (vec
                (last
                 (split-at
                  (- (count ls-log) 13)
