...
* [`src/re_frame_puzzle/`](src/re_frame_puzzle/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/re_frame_puzzle/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `re-frame-puzzle` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_puzzle/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-puzzle.example
  (:require [re-frame-puzzle.config :as config])
